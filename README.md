...
Этап 1. "Эксперимент". 
Для начала произвольно и незначительно поменяем параметры и посмотрим, что из этого выйдет. Отталкиваться на этом этапе будем от имеющегося графика в tensorflow. Идея такова: попробовать сломать все, что можно, а после выяснить каким образом оно ломается и хорошо ли это в нашей задаче.
...
- Да, параметры были изменены не совсем бессознательно, так как нужно знать, что вообще можно поменять, однако никакой общей идеи в этих изменениях не было. Проведем обучение до 25 тыс. шагов или до двух отчётов о полученном вознаграждении.
...
Теперь попробуем поставить идею повышения скорости обучаемости модели. То есть, я возвращаюсь к параметрам более близким к стандартным, но с уклоном в более быстрый процесс обучения. Для этого я приметил из документации несколько интересных парметров тренера(наш тип PPO).
...
## Выводы
В процессе выполнения 5 лабораторной работы я познакомился с инструментом tensorflow - графическим отображением поведения нейросети во время обучения, открытая программной библиотекой для машинного обучения, разработанная компанией Google для решения задач построения и тренировки нейронной сети с целью автоматического нахождения и классификации образов tensorflow. Применяется как для исследований, так и для разработки собственных продуктов Google.
Также мы повторно обратились к документации `ml-agent 19` для решения второго этапа нашей задачи. Это закрепило навыки, полученные при выполнении лабораторной работы №3 и создании RollerAgent.
